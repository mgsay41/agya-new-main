...
const CommentPopup = ({ isOpen, onClose, articleID }) => {
      ...
      if (isOpen && articleID) {
        try {
          ...
          const commentsRes = await fetch(
            `https://agya-backend.vercel.app/api/comments/article/${articleID}`
          );
          const commentsData = await commentsRes.json();
          setComments(commentsData);
          // Fetch replies
          const repliesRes = await fetch(
            `https://agya-backend.vercel.app/api/replies/${articleID}/replies`
          );
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    try {
      const response = await fetch(
        `https://agya-backend.vercel.app/api/replies/${commentId}/reply`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
                      ...
                      {commentReplies.map((reply) => (
                        ...
                        >
                          <img
                            ...
                            src={reply.user?.image || "./avatar.jpeg"}
                            alt="Profile"
                          />
                          <div className="flex-1">
                            <p className="font-semibold">
