...
const Navbar = () => {
  ...
  const clearAllNotifications = async () => {
    try {
      const response = await fetch(
        `https://agyademo.uber.space/api/notifications/${isAuthUser.id}/all`,
        {
          method: "DELETE",
          headers: {
  ...
  const markAsRead = async (id) => {
    try {
      const response = await fetch(
        `https://agyademo.uber.space/api/notifications/${id}/read`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    if (isAuthUser?.id) {
      ...
      const fetchNotifications = async () => {
        try {
          const response = await fetch(
            `https://agyademo.uber.space/api/notifications/${isAuthUser.id}`,
  ...
  useEffect(() => {
    fetch("https://agyademo.uber.space/api/tags/all")
  ...
  const newPost = async () => {
    ...
    try {
      ...
      toastBC.current.show({
        ...
        summary: data.message || "Post created successfully",
        sticky: true,
      });
      ...
      toastBC.current.show({
        severity: "error",
        summary: err.message || "An error occurred",
        sticky: true,
      });
    } finally {
      setLoading(false);
          ...
          <div className="flex-1 max-w-xl mx-auto hidden md:flex items-center gap-2">
            ...
            >
              ...
              <input
                ...
                name="search"
                className="w-full pl-10 pr-4 py-2 bg-main/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-main placeholder-gray-500"
